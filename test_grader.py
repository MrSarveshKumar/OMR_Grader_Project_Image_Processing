...
from openpyxl import Workbook, load_workbook #python library for working with excel files
import os #python library for working with file paths 
...
# Mapping answers 0, 1, 2, 3, 4 to 1, 2, 3, 4, 5 respectively
ANSWER_MAP = {0: 1, 1: 2, 2: 3, 3: 4, 4: 5}

# Extract student ID from the image filename
student_id = os.path.splitext(os.path.basename(args["image"]))[0].split("_")[-1]

# Initialize a list to store student's marked answers
marked_answers = []
...
# apply thresholding method to binarize the warped piece of paper
...
for (q, i) in enumerate(np.arange(0, len(questionCnts), 5)):
    ...
    # Add the mapped index of the bubbled answer to the list of marked answers
    marked_answers.append(ANSWER_MAP[bubbled[1]])
...
# Load existing workbook or create a new one if not exist
try:
...
#if ws["A1"].value != "Student ID":
    #ws.append(["Student ID", "Total Score", "Answer 1", "Answer 2", "Answer 3", "Answer 4", "Answer 5"])

# Append new student data without headers
ws.append([student_id, score] + marked_answers)
